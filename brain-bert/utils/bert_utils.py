...
def predict_bert_embeddings(words_in_array, tokenizer, model, remove_chars):    
    ...
    encoded_layers, _ = model(tokens_tensor) # retrurn output from 12 layers
    #print(len(encoded_layers)) # 12
    #print(encoded_layers[0].shape) # 1 59 786
    #print(model.pooler(encoded_layers[-1]).shape) # 1 786
    pooled_output = np.squeeze(model.pooler(encoded_layers[-1]).detach().numpy()) # 786,
...
if __name__ == '__main__':
    ...
    nlp_features = get_bert_layer_representations(seq_len, text_array, remove_chars, word_ind_to_extract)
    #print(text_array[0])
    #print(nlp_features[0])
